...
async function handleLogin() {
  ...
  loading.value = true;
  try {
    const response = await apiClient.post("/auth/super-admin", {
      email: email.value,
...
<template>
  <div
    class="min-h-screen flex items-center justify-center bg-gray-900 p-6 relative overflow-hidden"
  >
    <!-- Background Decor -->
    <div
    ...
    <div
      class="glass-card bg-gray-900 border border-gray-700 p-8 rounded-3xl w-full max-w-md shadow-2xl relative z-10"
    >
      ...
      <div class="space-y-4">
        <BaseInput
          v-model="email"
          label="Email Address"
          placeholder="admin@snaporder.com"
        ...
        <BaseButton
          class="w-full mt-4 bg-purple-600 hover:bg-purple-500 shadow-purple-900/50"
          :loading="loading"
          @click="handleLogin"
          size="lg"
        >
