...
const isValid = computed(() => {
  const received = parseFloat(receivedAmount.value);
  return !isNaN(received) && received >= props.totalAmount;
});
...
function exactAmount() {
  receivedAmount.value = Number(props.totalAmount).toFixed(2);
}
</script>
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div
      ...
      <div class="p-6">
        <div class="mb-6">
          <label class="block text-sm font-bold text-app-muted mb-2">Received Cash ($)</label>
        ...
        <div v-if="change > 0"
          ...
          <span class="text-2xl font-bold">${{ Number(change).toFixed(2) }}</span>
        </div>

        <!-- Numpad -->
        <div class="grid grid-cols-3 gap-3 mb-6">
          <button v-for="n in [1, 2, 3, 4, 5, 6, 7, 8, 9]" :key="n" @click="appendNumber(n.toString())"
        ...
        <button @click="handleConfirm" :disabled="!isValid" :class="[
